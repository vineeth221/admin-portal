...
const Products = () => {
  ...
  useEffect(() => {
    axios
      .get('/api/emails')
      .then((response) => {
        const emailsWithDisplayId = response.data.map((email, index) => ({
          ...email,
          displayId: index + 1, // Numbering starts from 1
          encryptedId: encryptId(email._id),
        }));
        dispatch(setEmails(emailsWithDisplayId));
      })
  ...
  const handleDeleteConfirm = async () => {
    try {
      await axios.delete(`/api/emails/${decryptId(selectedEmailId)}`);
  ...
  return (
    <div className="products-main-container relative">
      <div className="flex justify-between items-center mb-4" style={{display:"flex", alignItems:"center"}}>
